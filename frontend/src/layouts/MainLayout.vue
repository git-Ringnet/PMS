...
function handleHeaderColorChange(e) {
  headerBgColor.value = e.target.value
  localStorage.setItem('pms_header_bg_color', e.target.value)
}
...
<template>
  ...
  <div class="flex flex-col h-screen w-full max-w-full overflow-hidden bg-white">
    ...
    <header 
      ...
      :style="{ backgroundColor: (route.path !== '/pms' && route.path !== '/' && route.path !== '/login') ? headerBgColor : '' }"
    >
      <!-- Logo (Left) -->
      ...
      <div class="flex items-center justify-end gap-1.5 text-sm whitespace-nowrap shrink-0">
        ...
        <input 
          ...
          class="absolute opacity-0 pointer-events-none w-0 h-0 p-0 border-none"
        />
