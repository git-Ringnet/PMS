...
function handleFilterScopeChange() {
  if (filterDepartureChecked.value && filterDateScope.value !== 'custom') setFilterDatesForScope(filterDateScope.value)
}

watch(filterDepartureChecked, checked => {
  if (!checked) {
    filterDateScope.value = 'today'
    filterDateFrom.value = ''
    filterDateTo.value = ''
  } else if (!filterDateFrom.value || !filterDateTo.value) {
    setFilterDatesForScope(filterDateScope.value)
  }
})
...
const activeCheckoutGuests = room => (room.allGuests || []).filter(guest => guest?.id)
const checkoutGuestOptions = room => [
  ...activeCheckoutGuests(room),
  ...((room.rawRoom?.children || []).map(child => ({
    id: child.id,
    name: child.full_name || child.name || 'Trẻ em',
    isChild: true
  })))
]
const isCheckoutGuestSelected = (roomId, guestId) => (checkoutGuestSelections.value[roomId] || []).some(id => String(id) === String(guestId))
const toggleCheckoutGuest = (roomId, guestId, checked) => {
  const current = checkoutGuestSelections.value[roomId] || []
  checkoutGuestSelections.value = {
    ...checkoutGuestSelections.value,
    [roomId]: checked
      ? [...new Set([...current, guestId])]
      : current.filter(id => String(id) !== String(guestId))
  }
}
      ...
      if (b.booking_rooms && b.booking_rooms.length > 0) {
        b.booking_rooms.forEach(r => {
          ...
          if (r.guests && Array.isArray(r.guests) && r.guests.length > 0) {
            r.guests.forEach(g => {
              if ([3, 100].includes(Number(g.status)) || (Number(g.status) === 2 && activeFilter.register !== 'old')) return
          ...
          // Chỉ dùng guest_name legacy khi API không trả danh sách khách hợp lệ;
          // tránh hiển thị lại khách đã checkout trong r.guest_name.
          if (roomGuests.length === 0 && r.guest_name && r.guest_name.trim()) {
          ...
          // Luôn đưa khách chính hiện hữu của phòng lên đầu danh sách.
          roomGuests.sort((a, b) => Number(b.isPrimary) - Number(a.isPrimary))
          ...
          const masterUnpaidRoomChargeTotal = 0
          const masterRoomChargeTotal = 0
          ...
          roomGuests.forEach(guest => {
            ...
            allBookingBills.forEach(sb => {
              ...
              if (billBelongsToCurrentRoom(sb, r)) {
                const billGuestId = serviceBillCurrentGuestId(sb)
          ...
          roomItems.push({
            ...
            masterUnpaidRoomChargeAmount: masterUnpaidRoomChargeTotal,
            masterRoomChargeAmount: masterRoomChargeTotal,
...
const servicesList = computed(() => {
  ...
  const billOperatorName = (bill) => (
    bill?.employee_operator?.name
    || bill?.employeeOperator?.name
    || bill?.username_operator?.name
    || bill?.usernameOperator?.name
    || bill?.CreatedUser
    || bill?.Username
    || ''
  )
  ...
  const processServiceItem = (s, idx, defaultDesc, roomNo = '') => {
    ...
    return {
      ...
      userName: billOperatorName(linkedBill) || s.created_by || s.user_name || '',
  ...
  const processServiceBillRecord = (sb, idx) => {
    ...
    return {
      ...
      vatNo: '',
      accounting: 'Đã ghi',
      userName: billOperatorName(sb),
      guestId: serviceBillCurrentGuestId(sb)
    }
  }
  ...
  if (selectedRoomItem.value) {
    ...
    ;(room.rawRoom?.services || []).forEach((service, idx) => {
      ...
      const belongsToGuest = !filterByGuest || (serviceGuestId
        ? String(serviceGuestId) === String(guestId)
        : isPrimary)
    ...
    if (rawB) {
      ...
      allBills.forEach((sb, idx) => {
        ...
        const billGuestId = serviceBillCurrentGuestId(sb)
        const belongsToSelectedGuest = !filterByGuest || (billGuestId
...
const compareServiceDateTime = (left, right) => {
  const leftDate = normalizeServiceDate(left.serviceDate || left.createdAt) || '9999-12-31'
  const rightDate = normalizeServiceDate(right.serviceDate || right.createdAt) || '9999-12-31'
  const dateOrder = leftDate.localeCompare(rightDate)
  if (dateOrder !== 0) return dateOrder

  const leftTime = formatTime(left.serviceTime) || formatTime(left.createdAt) || '00:00'
  const rightTime = formatTime(right.serviceTime) || formatTime(right.createdAt) || '00:00'
  const timeOrder = leftTime.localeCompare(rightTime)
  if (timeOrder !== 0) return timeOrder

  return String(left.serviceBillId || left.id || '').localeCompare(
    String(right.serviceBillId || right.id || ''),
    undefined,
    { numeric: true }
  )
}
...
const filteredServiceItems = computed(() => {
  ...
  const services = filter ? servicesList.value.filter(service => {
  ...
  return [...services].sort(compareServiceDateTime)
})
...
const getGuestsToDisplay = (b, r) => {
  ...
  const roomBills = mergeServiceBills(rawR?.service_bills || [], rawR?.current_service_bills || [])
  const allBills = mergeServiceBills(rawB?.service_bills || [], rawB?.master_service_bills || [], roomBills)

  r.allGuests.slice(1).forEach(g => {
    if (!g.id) return
    const hasService = (rawR?.services || []).some(s => String(s.guest_id) === String(g.id))
    const hasBill = allBills.some(bill => (
      Number(bill.Edit) !== 1
      && billBelongsToCurrentRoom(bill, r)
      && String(serviceBillCurrentGuestId(bill)) === String(g.id)
    ))
    ...
    if (hasService || hasBill || hasPayment) {
      list.push(g)
    }
  })
...
const masterTransferPreviewServices = (booking) => {
  const masterBills = previewBookingBills(booking).filter(bill => (
    ...
    && isMasterBillRecord(bill, booking.rawBooking)
  ))
            ...
            <div v-if="showRegisterFilterDropdown" class="checkout-filter-dropdown" @click.stop>
              ...
              <div class="checkout-filter-box">
                <div class="checkout-filter-box-title">Phạm vi ngày</div>
                                <select v-model="filterDateScope" @change="handleFilterScopeChange" :disabled="!filterDepartureChecked" class="checkout-filter-scope">
          ...
          <template v-else-if="selectedCheckoutRooms.length > 1">
            ...
            <div class="max-h-[320px] space-y-1 overflow-y-auto pr-1">
              <div v-for="selection in selectedCheckoutRooms" :key="selection.room.id" class="rounded border px-3 py-2 text-xs text-slate-700">
                <div class="mb-1 font-semibold">- Phòng {{ selection.room.roomNumber || 'ảo' }}</div>
                <label v-for="guest in checkoutGuestOptions(selection.room)" :key="`${selection.room.id}-${guest.id}`" class="mb-1 flex items-center justify-between rounded border px-2 py-1.5 last:mb-0">
                  <span>{{ guest.name }}</span>
                  <input type="checkbox" :checked="guest.isChild || isCheckoutGuestSelected(selection.room.id, guest.id)" @change="toggleCheckoutGuest(selection.room.id, guest.id, $event.target.checked)" class="h-4 w-4 accent-sky-500" />
                </label>
              </div>
            </div>
          </template>
