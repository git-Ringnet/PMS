...
const props = defineProps({ show: Boolean, loading: Boolean, count: { type: Number, default: 0 } })
...
<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-md overflow-hidden rounded-xl border border-sky-500 bg-white text-xs shadow-2xl">
      <header class="flex items-center justify-between bg-[#0788f5] px-4 py-2 text-white"><span class="font-bold">Xóa dịch vụ</span><button :disabled="loading" class="rounded transition hover:bg-white/20 active:scale-90" @click="emit('close')"><X class="h-5 w-5" /></button></header>
      ...
      <footer class="flex justify-end gap-2 border-t border-gray-300 bg-gray-50 p-3"><button :disabled="loading" class="rounded bg-gray-400 px-4 py-1.5 font-bold text-white transition hover:bg-gray-500 active:scale-95 disabled:opacity-40" @click="emit('close')">Đóng</button><button :disabled="loading || !reason.trim()" class="rounded bg-red-500 px-4 py-1.5 font-bold text-white transition hover:bg-red-600 active:scale-95 disabled:opacity-40" @click="submit"><Trash2 class="mr-1 inline h-4 w-4" />{{ loading ? 'Đang xóa...' : 'Xóa dịch vụ' }}</button></footer>
    </div>
  </div>
</template>
