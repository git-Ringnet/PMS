      ...
      <nav class="flex-1 py-2 px-2 flex flex-col gap-0.5">
        ...
        >
          ...
          <span
            ...
            class="absolute left-0 top-1 bottom-1 w-[3px] rounded-r-full bg-sky-500"
          ></span>

          <!-- Icon -->
          <svg v-if="item.icon === 'file-text'" class="w-[18px] h-[18px] shrink-0 transition-colors" :class="activeTab === item.id ? 'text-sky-500' : 'text-slate-400 group-hover:text-slate-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
          </svg>
          <svg v-if="item.icon === 'box'" class="w-[18px] h-[18px] shrink-0 transition-colors" :class="activeTab === item.id ? 'text-sky-500' : 'text-slate-400 group-hover:text-slate-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
          </svg>
          <svg v-if="item.icon === 'printer'" class="w-[18px] h-[18px] shrink-0 transition-colors" :class="activeTab === item.id ? 'text-sky-500' : 'text-slate-400 group-hover:text-slate-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/>
          </svg>
          <svg v-if="item.icon === 'settings'" class="w-[18px] h-[18px] shrink-0 transition-colors" :class="activeTab === item.id ? 'text-sky-500' : 'text-slate-400 group-hover:text-slate-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <span
            ...
            class="text-[10px] px-1.5 py-0.5 rounded border font-semibold shrink-0 transition-colors"
