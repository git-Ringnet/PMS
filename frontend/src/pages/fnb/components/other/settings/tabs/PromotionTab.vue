...
const promotions = ref([
  { id: 1, name: 'FOC', startDate: '07/11/2025', endDate: '07/11/2030', status: 'Đang áp dụng' }
])
    ...
    <div class="flex-1 overflow-auto flex flex-col">
      <table class="w-full text-sm border-collapse whitespace-nowrap text-left text-slate-600">
        ...
        <tbody class="bg-white">
          <tr v-for="item in promotions" :key="item.id" class="border-b border-slate-100 hover:bg-slate-50/60 transition-colors">
            <td class="px-6 py-3 font-medium text-slate-800 border-r border-slate-100">{{ item.name }}</td>
            <td class="px-6 py-3 text-center border-r border-slate-100">{{ item.startDate }}</td>
            <td class="px-6 py-3 text-center border-r border-slate-100">{{ item.endDate }}</td>
            <td class="px-6 py-3 text-center text-sky-500 font-medium">
              <span class="px-2 py-1 bg-sky-50 text-sky-500 border border-sky-200 rounded text-xs">{{ item.status }}</span>
            </td>
