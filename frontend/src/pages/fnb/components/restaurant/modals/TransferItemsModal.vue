...
const validateQuantity = (item) => {
  if (!canEditQuantity(item)) {
    item.transferQty = item.quantity
    return
  }
  let val = parseInt(item.transferQty)
  if (isNaN(val) || val < 1) val = 1
  if (val > item.quantity) val = item.quantity
  item.transferQty = val
}
...
const selectTargetTable = (table) => {
  ...
  if (table.status !== 'empty' && table.status !== 'serving') return // Only allow transfer to empty tables
    ...
    <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div class="bg-white rounded-lg shadow-xl w-[90vw] max-w-5xl h-[80vh] flex flex-col relative overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div class="flex-1 flex overflow-hidden">
          ...
          <!-- Right Sidebar (Items to transfer) -->
          <div class="w-72 border-l border-slate-200 bg-[#f8f9fa] flex flex-col shrink-0 z-10 relative shadow-[-2px_0_8px_rgba(0,0,0,0.02)]">
            <div class="p-3 bg-white border-b border-slate-100 flex items-center justify-between shrink-0">
              <span class="text-xs font-bold text-slate-700 uppercase tracking-wide">Số lượng chuyển</span>
              <span class="text-[10px] bg-sky-100 text-sky-700 px-2 py-0.5 rounded-full font-bold">{{ localItems.length }} món</span>
            </div>
            <div class="flex-1 overflow-y-auto p-3 space-y-3 scrollbar-thin">
              <div v-for="item in localItems" :key="item.id" class="border border-slate-200 rounded-[10px] p-2.5 bg-white shadow-sm flex flex-col gap-2 relative overflow-hidden group hover:border-sky-300 transition-colors">
                <div class="font-bold text-slate-700 text-xs line-clamp-2 leading-tight">
                  {{ item.name || item.product?.name || item.product_name }}
                </div>
                <div class="flex items-center justify-between mt-1">
                  <div class="text-[10px] font-semibold text-slate-500 flex flex-col">
                    <span>Gốc: <span class="text-slate-700">{{ item.quantity }}</span></span>
                    <span v-if="!canEditQuantity(item)" class="text-amber-600 mt-0.5 text-[8px]">(Không thể chia nhỏ)</span>
                  </div>
                  <div class="flex items-center gap-1.5">
                    <button 
                      @click="item.transferQty = Math.max(1, item.transferQty - 1)" 
                      :disabled="!canEditQuantity(item) || item.transferQty <= 1"
                      class="w-6 h-6 rounded-md bg-slate-100 text-slate-600 flex items-center justify-center hover:bg-slate-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                    ...
                    <input 
                      ...
                      :max="item.quantity"
                      :min="canEditQuantity(item) ? 1 : item.quantity"
                      :disabled="!canEditQuantity(item)"
                      @change="validateQuantity(item)"
                      class="w-10 h-6 border border-slate-200 rounded-md text-center text-xs font-bold text-sky-600 focus:outline-none focus:border-sky-400 focus:ring-1 focus:ring-sky-400 disabled:bg-slate-50 disabled:text-slate-400"
                    />
                    <button 
                      @click="item.transferQty = Math.min(item.quantity, item.transferQty + 1)" 
                      :disabled="!canEditQuantity(item) || item.transferQty >= item.quantity"
                      class="w-6 h-6 rounded-md bg-slate-100 text-slate-600 flex items-center justify-center hover:bg-slate-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
              ...
              <div v-if="localItems.length === 0" class="text-center text-slate-400 text-xs italic mt-4 w-full">Chưa chọn món nào</div>
            </div>
          </div>
        </div>
