...
import { fetchCustomerSources, createCustomerSource, updateCustomerSource, deleteCustomerSource } from '@/services/company-service'
...
const filteredAndSortedSources = computed(() => {
  ...
  if (searchQueryCode.value.trim()) {
    ...
    result = result.filter(s => s.code && s.code.toLowerCase().includes(q))
  }
...
const displayedSources = computed(() => filteredAndSortedSources.value)

const loadData = async () => {
  loading.value = true
  try {
    const res = await fetchCustomerSources()
...
const saveItem = async () => {
  ...
  try {
    if (isEditMode.value) {
      await updateCustomerSource(currentId.value, form.value)
      uiStore.showToast('Cập nhật nguồn khách thành công!', 'success')
    } else {
      await createCustomerSource(form.value)
...
const handleDelete = async (item) => {
  const confirmed = await uiStore.confirm({
    title: 'Xác nhận xóa',
    message: `Bạn có chắc chắn muốn xóa nguồn khách "${item.name}"?`,
    confirmText: 'Xóa',
    cancelText: 'Hủy'
  })
  if (!confirmed) return
  ...
  try {
    await deleteCustomerSource(item.id)
    uiStore.showToast('Xóa nguồn khách thành công!', 'success')
    loadData()
  } catch (err) {
    console.error(err)
  ...
  <div class="p-3 bg-white flex-1 flex flex-col overflow-hidden text-xs">
    ...
    <div class="overflow-auto border border-slate-200 rounded-lg shadow-sm flex-1 max-h-full">
      <table class="w-full text-left border-collapse text-xs">
        ...
        <tbody>
          <tr 
            v-for="item in displayedSources" 
            :key="item.id" 
          ...
          <tr v-if="displayedSources.length === 0 && !loading">
            <td colspan="4" class="p-8 text-center text-slate-400 text-xs font-semibold">Chưa có dữ liệu nguồn khách</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <!-- Pagination -->
    <div v-if="sources.length > 0" class="flex items-center justify-end mt-3 gap-1 select-none">
  ...
  >
    <div class="bg-white rounded-xl w-full max-w-sm shadow-2xl overflow-hidden border border-slate-100 animate-in">
      <div class="bg-[#8dcbf4] px-5 py-3 flex items-center justify-between text-white select-none">
        <h2 class="text-xs font-bold uppercase tracking-wider">{{ isEditMode ? 'Sửa nguồn khách' : 'Thêm nguồn khách' }}</h2>
      ...
      <div class="p-5 flex flex-col gap-3 text-xs">
        <div class="flex flex-col gap-1">
          ...
          <input v-model="form.code" type="text" placeholder="Nhập mã..." class="border border-slate-200 bg-[#fffbeb] rounded-md p-1.5 font-bold focus:outline-sky-500 text-xs uppercase" />
        </div>
        <div class="flex flex-col gap-1">
