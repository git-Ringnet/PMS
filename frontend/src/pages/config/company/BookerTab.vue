...
const filteredAndSortedBookers = computed(() => {
  let result = [...bookers.value]
...
const displayedBookers = computed(() => filteredAndSortedBookers.value)

const loadData = async () => {
  loading.value = true
  try {
    const res = await fetchBookers()
    bookers.value = res.data.data || []
  } catch (err) {
    console.error(err)
  } finally {
    loading.value = false
...
const saveItem = async () => {
  ...
  try {
    if (isEditMode.value) {
      await updateBooker(currentId.value, form.value)
      uiStore.showToast('Cập nhật người đặt phòng thành công!', 'success')
    } else {
      await createBooker(form.value)
...
const handleDelete = async (item) => {
  const confirmed = await uiStore.confirm({
    title: 'Xác nhận xóa',
    message: `Bạn có chắc chắn muốn xóa người đặt phòng "${item.name}"?`,
    confirmText: 'Xóa',
    cancelText: 'Hủy'
  })
  if (!confirmed) return
  ...
  try {
    await deleteBooker(item.id)
    uiStore.showToast('Xóa người đặt phòng thành công!', 'success')
    loadData()
  } catch (err) {
    console.error(err)
  ...
  <div class="p-3 bg-white flex-1 flex flex-col overflow-hidden text-xs">
    ...
    <div class="overflow-auto border border-slate-200 rounded-lg shadow-sm flex-1 max-h-full">
      <table class="w-full text-left border-collapse text-xs">
        ...
        <tbody>
          <tr 
            v-for="item in displayedBookers" 
            :key="item.id" 
          ...
          <tr v-if="displayedBookers.length === 0 && !loading">
            <td colspan="7" class="p-8 text-center text-slate-400 text-xs font-semibold">Chưa có dữ liệu người đặt phòng</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <!-- Pagination -->
    <div v-if="bookers.length > 0" class="flex items-center justify-end mt-3 gap-1 select-none">
