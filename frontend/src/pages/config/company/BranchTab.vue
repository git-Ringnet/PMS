      ...
      <table class="w-full text-left border-collapse text-xs">
        <thead>
          <tr class="bg-slate-100 border-b border-slate-200 text-slate-700 font-bold select-none h-9">
            <!-- Mã / ID -->
            <th class="p-2 border-r border-slate-200 text-slate-700 font-bold text-xs uppercase w-20 cursor-pointer hover:bg-slate-200 select-none relative popover-container transition-colors">
  ...
  >
    <div class="bg-white rounded-xl w-full max-w-sm shadow-2xl overflow-hidden border border-slate-100 animate-in">
      <div class="bg-[#8dcbf4] px-5 py-3 flex items-center justify-between text-white select-none">
        <h2 class="text-xs font-bold uppercase tracking-wider">{{ isEditMode ? 'Sửa chi nhánh' : 'Thêm chi nhánh' }}</h2>
      ...
      <div class="p-5 flex flex-col gap-3 text-xs">
        <div class="flex flex-col gap-1">
          ...
          <input v-model="form.name" type="text" placeholder="Nhập tên chi nhánh..." class="border border-slate-200 bg-[#fffbeb] rounded-md p-1.5 font-semibold focus:outline-sky-500 text-xs" />
        </div>
      </div>
