...
// 4. Room Rate Code Modal Actions
const openAddRate = () => {
  isEditMode.value = false
...
// 5. Registration Status Modal Actions
const openAddStatus = () => {
  isEditMode.value = false
...
const paginatedRates = computed(() => {
  const start = (pagination.rate.page - 1) * pagination.rate.limit
  return filteredRates.value.slice(start, start + pagination.rate.limit)
})
const totalRatePages = computed(() => Math.ceil(filteredRates.value.length / pagination.rate.limit) || 1)
...
<template>
  <div class="flex flex-col gap-4 h-full overflow-hidden">
    ...
    <div class="flex-1 bg-white rounded-xl shadow-xs border border-slate-200 p-6 overflow-y-auto min-h-[400px] relative">
      ...
      <!-- Tab 4: MÃ GIÁ PHÒNG -->
      <div v-if="activeSystemTab === 'MÃ GIÁ PHÒNG'" class="flex flex-col gap-4">
        <div class="flex justify-between items-center mb-2 flex-wrap gap-2">
          ...
          <div class="relative max-w-xs w-full">
            <input type="text" v-model="searchQueries.rate" placeholder="Tìm kiếm mã, mô tả..."
              class="w-full border border-slate-200 rounded-lg pl-9 pr-3 py-2 text-sm font-semibold focus:outline-sky-500 focus:bg-white" />
            <svg class="w-4 h-4 text-slate-400 absolute left-3 top-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
        ...
        <!-- Pagination -->
        <div class="flex items-center justify-end gap-2 mt-4 select-none">
          <button @click="pagination.rate.page = Math.max(1, pagination.rate.page - 1)" :disabled="pagination.rate.page === 1"
            class="w-8 h-8 flex items-center justify-center border border-slate-200 bg-white rounded text-slate-500 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer font-bold">&lt;</button>
          <button v-for="p in totalRatePages" :key="p" @click="pagination.rate.page = p"
            class="w-8 h-8 flex items-center justify-center border rounded font-semibold cursor-pointer text-sm"
            :class="pagination.rate.page === p ? 'border-sky-500 text-sky-600 font-bold bg-sky-50/20' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'">{{ p }}</button>
          <button @click="pagination.rate.page = Math.min(totalRatePages, pagination.rate.page + 1)" :disabled="pagination.rate.page === totalRatePages"
            class="w-8 h-8 flex items-center justify-center border border-slate-200 bg-white rounded text-slate-500 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer font-bold">&gt;</button>
        </div>
      </div>

      <!-- Tab 5: TÌNH TRẠNG ĐĂNG KÝ -->
