...
const availableRatePlans = computed(() => {
  ...
  return selectedRateCode.value.rate_plans
  ...
  try {
    ...
    if (res.status === 200 || res.status === 201) {
      ...
      if (!rateFormState.isDaily) {
        ...
        // Save Daily Mappings
        if (dailyMappingsList.value.length > 0) {
...
<template>
  <div class="flex-1 flex flex-col gap-4 text-slate-800 min-h-0 overflow-y-auto">
    ...
    <div class="flex-1 min-h-0 flex flex-col gap-4">
      ...
      <div v-if="activeRateTab === 'Mã giá phòng'" class="flex-1 flex flex-col gap-4 min-h-0">
              ...
              <table class="w-full text-xs text-left border-collapse border border-slate-200">
                ...
                <tbody>
                  <tr 
                    ...
                    :class="selectedRateCode?.Ma === rc.Ma ? 'bg-sky-50/40 font-semibold text-sky-700' : 'text-slate-700'"
                  >
                    ...
                    <td class="p-2 border border-slate-100 text-center text-slate-500">{{ rc.BeginDate }}</td>
                    <td class="p-2 border border-slate-100 text-center text-slate-500">{{ rc.EndDate }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div class="w-full lg:w-[58%] bg-white border border-slate-200 rounded-xl p-4 flex flex-col min-h-0 shadow-xs justify-between">
            ...
            <div class="flex items-center justify-between pb-3 border-b border-slate-100 shrink-0">
              ...
              <div class="flex items-center gap-1.5">
                ...
                <!-- Add Button -->
                <button @click="handleAdd" class="px-3 py-1.5 bg-[#8dcbf4] hover:bg-[#78bce8] text-white rounded-lg text-xs font-bold border-none cursor-pointer flex items-center gap-1 shadow-sm">
                  <span class="text-sm">+</span> Thêm
                </button>
                <!-- Save Button -->
                <button @click="handleSave" class="px-3 py-1.5 bg-sky-500 hover:bg-sky-600 text-white rounded-lg text-xs font-bold border-none cursor-pointer flex items-center gap-1 shadow-sm">
                  {{ isEditing ? 'Cập nhật' : 'Lưu' }}
                </button>
                <!-- Delete Button -->
                <button @click="handleDelete" class="px-3 py-1.5 btn-delete rounded-lg text-xs font-bold cursor-pointer flex items-center gap-1 shadow-sm">
                  Xóa
                </button>
              </div>
            </div>
            ...
            <div class="flex flex-col gap-3 py-3 overflow-y-auto flex-1">
              ...
              <div class="flex gap-4 items-end">
                <div class="w-[45%] flex gap-2">
                  <div class="w-1/2">
                    ...
                    <input type="date" v-model="rateFormState.BeginDate" class="w-full px-2 py-1.5 border border-slate-200 rounded-lg text-xs focus:outline-sky-400" />
                  </div>
                  <div class="w-1/2">
                    <label class="block text-xs font-bold text-slate-500 mb-1">Đến ngày</label>
                    <input type="date" v-model="rateFormState.EndDate" class="w-full px-2 py-1.5 border border-slate-200 rounded-lg text-xs focus:outline-sky-400" />
                  </div>
                </div>
                ...
                <div class="w-[25%]">
                  ...
                  >
                    ...
                    <span class="font-bold text-slate-700">{{ rateFormState.Currency }}</span>
                    <svg class="w-3.5 h-3.5 text-slate-400 ml-auto" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
                  </button>
                </div>

                <div class="w-[30%] pb-2 pl-2">
                  <label class="inline-flex items-center gap-1.5 text-xs text-slate-600 cursor-pointer select-none">
                    <input type="checkbox" v-model="rateFormState.IncludeBF" class="rounded border-slate-300 text-sky-500 focus:ring-sky-400" />
                    Ăn sáng
                  </label>
                </div>
              </div>
              ...
              <div class="flex flex-wrap gap-x-5 gap-y-2 items-center pt-2 border-t border-slate-100">
                ...
                <label class="inline-flex items-center gap-1.5 text-xs text-slate-600 cursor-pointer select-none">
                  <input type="checkbox" v-model="rateFormState.Disable" class="rounded border-slate-300 text-sky-500 focus:ring-sky-400" />
                  Không sử dụng
                </label>
                <label class="inline-flex items-center gap-1.5 text-xs text-slate-600 cursor-pointer select-none">
                  <input type="checkbox" v-model="rateFormState.AllowChangeRate" class="rounded border-slate-300 text-sky-500 focus:ring-sky-400" />
                  Cho phép nhập giá
                </label>
                <label class="inline-flex items-center gap-1.5 text-xs text-slate-600 cursor-pointer select-none">
                  <input type="checkbox" v-model="rateFormState.IsChannelManager" class="rounded border-slate-300 text-sky-500 focus:ring-sky-400" />
              ...
              <div v-if="rateFormState.isDaily" class="flex flex-col gap-3 pt-2 border-t border-slate-200">
                <div class="flex gap-4 items-end flex-wrap">
                  <div class="flex items-center gap-2">
                    <label class="text-xs font-bold text-slate-500 whitespace-nowrap">Ngày áp dụng</label>
                    <input type="date" v-model="batchFromDate" class="border border-slate-200 px-2 py-1.5 rounded text-xs focus:outline-sky-400" />
                    <span class="text-xs text-slate-400">~</span>
                    <input type="date" v-model="batchToDate" class="border border-slate-200 px-2 py-1.5 rounded text-xs focus:outline-sky-400" />
                  </div>
                </div>
                <div class="flex items-center gap-3 flex-wrap">
                  <label class="inline-flex items-center gap-1 text-xs font-semibold text-sky-600 cursor-pointer"><input type="checkbox" v-model="batchDaysOfWeek[1]" class="rounded text-sky-500" />T2</label>
                  <label class="inline-flex items-center gap-1 text-xs font-semibold text-sky-600 cursor-pointer"><input type="checkbox" v-model="batchDaysOfWeek[2]" class="rounded text-sky-500" />T3</label>
                  <label class="inline-flex items-center gap-1 text-xs font-semibold text-sky-600 cursor-pointer"><input type="checkbox" v-model="batchDaysOfWeek[3]" class="rounded text-sky-500" />T4</label>
                  <label class="inline-flex items-center gap-1 text-xs font-semibold text-sky-600 cursor-pointer"><input type="checkbox" v-model="batchDaysOfWeek[4]" class="rounded text-sky-500" />T5</label>
                  <label class="inline-flex items-center gap-1 text-xs font-semibold text-sky-600 cursor-pointer"><input type="checkbox" v-model="batchDaysOfWeek[5]" class="rounded text-sky-500" />T6</label>
                  <label class="inline-flex items-center gap-1 text-xs font-semibold text-sky-600 cursor-pointer"><input type="checkbox" v-model="batchDaysOfWeek[6]" class="rounded text-sky-500" />T7</label>
                  <label class="inline-flex items-center gap-1 text-xs font-semibold text-sky-600 cursor-pointer"><input type="checkbox" v-model="batchDaysOfWeek[0]" class="rounded text-sky-500" />CN</label>

                  <div class="flex items-center gap-2 border-l border-slate-300 pl-3 ml-2">
                    <span class="text-xs font-bold text-slate-500">Loại giá</span>
                    <select v-model="batchRateType" class="border border-slate-200 px-2 py-1.5 rounded text-xs focus:outline-sky-400 w-28 bg-white font-bold text-sky-700">
                      <option v-if="availableRatePlans.length === 0" value="" disabled>Trống</option>
                      <option v-for="plan in availableRatePlans" :key="plan.Code" :value="plan.Code">{{ plan.Code }}</option>
                    </select>
                    <button @click="openRatePlanModal" class="bg-sky-400 hover:bg-sky-500 text-white w-6 h-6 rounded-full flex items-center justify-center font-bold text-sm" title="Quản lý các loại giá">+</button>
                  </div>

                  <button @click="applyBatchUpdate" class="ml-auto px-4 py-1.5 bg-sky-500 hover:bg-sky-600 text-white rounded-lg text-xs font-bold border-none cursor-pointer shadow-sm">
                    Áp dụng
                  </button>
                </div>
              </div>
        ...
        <div v-if="!rateFormState.isDaily" class="h-[520px] shrink-0 bg-white border border-slate-200 rounded-xl p-4 flex flex-col shadow-xs">
          ...
          <div class="flex-1 overflow-auto">
            <table class="text-xs text-left border-collapse min-w-[1200px] border border-slate-200">
              <thead class="bg-slate-100 sticky top-0 z-20">
                ...
                <tr>
                  <th rowspan="2" class="p-2 border border-slate-200 w-28 text-center bg-slate-100 sticky left-0 z-30">Ngày</th>
                  <th rowspan="2" class="p-2 border border-slate-200 w-10 text-center bg-slate-100"></th>
                  <th rowspan="2" class="p-2 border border-slate-200 w-16 text-center bg-slate-100">Mã</th>
                  <th v-for="rt in roomTypes" :key="rt.code" :colspan="occupancies.length" class="p-2 border border-slate-200 text-center font-black text-slate-700 bg-slate-100">
                    {{ rt.code }}
                  </th>
                </tr>
                ...
                <tr>
                  <template v-for="rt in roomTypes" :key="'occ-' + rt.code">
                    <th v-for="occ in occupancies" :key="rt.code + '-' + occ" class="p-1.5 border border-slate-200 text-center text-[10px] font-semibold text-slate-500 bg-slate-50 w-20">
                      {{ occ }}
                    </th>
                  </template>
                </tr>
              ...
              <tbody>
                ...
                >
                  <td class="p-2 border border-slate-100 font-bold text-center whitespace-nowrap sticky left-0 bg-white z-10" :class="{ 'bg-sky-50/30': getDayOfWeekFromDate(mapping.Date) === 0 }">
                    {{ formatDateVN(mapping.Date) }}
                  </td>
                  <td class="p-2 border border-slate-100 text-center font-bold text-[10px]" :class="{ 'text-rose-500': getDayOfWeekFromDate(mapping.Date) === 0 || getDayOfWeekFromDate(mapping.Date) === 6 }">
                    {{ getDayLabel(getDayOfWeekFromDate(mapping.Date)) }}
                  </td>
                  <td class="p-2 border border-slate-100 text-center font-bold text-sky-700">
                    {{ mapping.Code }}
                  </td>
                ...
                <tr v-if="dailyMappingsList.length === 0">
                  <td :colspan="3 + roomTypes.length * occupancies.length" class="p-8 text-center text-slate-400 font-medium">
                    Chưa có ngày nào được thiết lập giá. Vui lòng nhập "Ngày áp dụng" ở trên và nhấn "Áp dụng".
                  </td>
                </tr>
              </tbody>
            </table>
    ...
    <Teleport to="body">
      <div v-if="showRatePlanModal" class="fixed inset-0 z-[9999] flex items-center justify-center p-6">
        ...
        <!-- Modal -->
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-6xl h-full max-h-[90vh] flex flex-col overflow-hidden">
          <!-- Header -->
          <div class="flex items-center justify-between px-5 py-3.5 bg-sky-400 text-white shrink-0">
            <span class="font-bold text-sm">Quản lý Các Loại Giá</span>
            <button @click="closeRatePlanModal" class="w-7 h-7 flex items-center justify-center rounded-full hover:bg-white/20 transition-colors cursor-pointer bg-transparent border-none text-white">
          ...
          <div class="flex h-[280px] border-b border-slate-200 shrink-0">
            ...
            <div class="w-1/2 border-r border-slate-200 overflow-y-auto p-4 bg-slate-50/50">
              <table class="w-full text-xs text-left border-collapse border border-slate-200 bg-white shadow-sm">
                <thead class="bg-slate-100/80 sticky top-0">
                  <tr>
                    <th class="p-2 border border-slate-200 text-slate-600 font-bold">Mã</th>
                    <th class="p-2 border border-slate-200 text-slate-600 font-bold">Mô tả</th>
                    <th class="p-2 border border-slate-200 text-slate-600 font-bold text-center">Từ ngày</th>
                    <th class="p-2 border border-slate-200 text-slate-600 font-bold text-center">Đến ngày</th>
                  </tr>
                </thead>
                <tbody>
                  <tr 
                    v-for="plan in modalRatePlans" 
                    :key="plan.Code"
                    @click="selectModalPlan(plan)"
                    class="cursor-pointer hover:bg-sky-50 transition-colors"
                    :class="{'bg-sky-100': modalFormState.Code === plan.Code}"
                  >
                    <td class="p-2 border border-slate-200 font-bold text-slate-700">{{ plan.Code }}</td>
                    <td class="p-2 border border-slate-200 text-slate-600">{{ plan.Description }}</td>
                    <td class="p-2 border border-slate-200 text-center text-slate-600">{{ formatDateVN(plan.BeginDate) }}</td>
                    <td class="p-2 border border-slate-200 text-center text-slate-600">{{ formatDateVN(plan.EndDate) }}</td>
                  </tr>
                  <tr v-if="modalRatePlans.length === 0">
                    <td colspan="4" class="p-6 text-center text-slate-400">Chưa có loại giá nào</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div class="w-1/2 p-5 flex flex-col">
              <div class="flex justify-end gap-2 mb-6">
                <button @click="resetModalForm" class="px-4 py-1.5 rounded-md text-xs font-bold border border-slate-200 hover:bg-slate-50 text-slate-600 transition-colors cursor-pointer">Thêm Mới</button>
                <button @click="saveModalPlan" class="px-5 py-1.5 bg-sky-400 hover:bg-sky-500 text-white rounded-md text-xs font-bold border-none transition-colors cursor-pointer shadow-sm">
                  {{ isModalEditing ? 'Cập nhật' : 'Lưu' }}
                </button>
                <button 
                  v-if="isModalEditing" 
                  @click="deleteModalPlan" 
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="flex flex-col gap-1.5">
                  <label class="text-xs font-bold text-slate-600">Mã</label>
                  <input type="text" v-model="modalFormState.Code" :disabled="isModalEditing" class="border border-slate-200 rounded p-1.5 text-xs focus:outline-sky-400 disabled:bg-slate-100" />
                </div>
                <div class="flex flex-col gap-1.5">
                  <label class="text-xs font-bold text-slate-600">Mô tả</label>
                  <input type="text" v-model="modalFormState.Description" class="border border-slate-200 rounded p-1.5 text-xs focus:outline-sky-400" />
                </div>
              </div>

              <div class="flex flex-col gap-1.5 mt-4">
                <label class="text-xs font-bold text-slate-600">Từ ngày - đến ngày</label>
                <div class="flex items-center gap-2">
                  <input type="date" v-model="modalFormState.BeginDate" class="border border-slate-200 rounded p-1.5 text-xs focus:outline-sky-400 w-full" />
                  <span class="text-slate-400">~</span>
                  <input type="date" v-model="modalFormState.EndDate" class="border border-slate-200 rounded p-1.5 text-xs focus:outline-sky-400 w-full" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex-1 relative bg-white flex flex-col min-h-0">
            <div v-if="!modalFormState.Code" class="absolute inset-0 z-20 bg-white/60 backdrop-blur-[2px] flex items-center justify-center pointer-events-none">
              <span class="bg-slate-800 text-white px-4 py-2 rounded-lg font-semibold text-sm shadow-xl pointer-events-auto">Vui lòng nhập Mã loại giá để tiếp tục</span>
            </div>
            <div class="flex-1 p-4 overflow-auto">
              <table class="w-full text-xs text-left border-collapse min-w-[800px] border border-slate-200">
              <thead>
                <tr class="bg-slate-100 border-b border-slate-200 text-slate-700 font-bold sticky top-0 z-10 text-center">
              ...
              <tbody>
                <tr v-for="rt in roomTypes" :key="rt.code" class="hover:bg-slate-50/50">
                  <td class="p-2 font-bold text-slate-700 border border-slate-200 bg-slate-50">{{ rt.code }}</td>
                  <td class="p-2 text-slate-600 border border-slate-200">{{ rt.description }}</td>
                  
                  <td class="p-1 border border-slate-200">
                    <input type="text" :disabled="!modalFormState.Code" :value="formatCurrencyInput(modalRateMatrix[getModalMatrixKey(rt.code, 'Double')], selectedRateCode?.Currency)" @input="e => modalRateMatrix[getModalMatrixKey(rt.code, 'Double')] = cleanCurrencyValue(e.target.value, selectedRateCode?.Currency)" class="w-full px-2 py-1.5 border border-slate-100 hover:border-slate-300 focus:border-sky-300 rounded text-center text-xs focus:outline-none font-semibold transition-colors disabled:bg-slate-50 disabled:text-slate-400 disabled:cursor-not-allowed" />
                  </td>
                  <td class="p-1 border border-slate-200">
                    <input type="text" :disabled="!modalFormState.Code" :value="formatCurrencyInput(modalRateMatrix[getModalMatrixKey(rt.code, 'Twin')], selectedRateCode?.Currency)" @input="e => modalRateMatrix[getModalMatrixKey(rt.code, 'Twin')] = cleanCurrencyValue(e.target.value, selectedRateCode?.Currency)" class="w-full px-2 py-1.5 border border-slate-100 hover:border-slate-300 focus:border-sky-300 rounded text-center text-xs focus:outline-none font-semibold transition-colors disabled:bg-slate-50 disabled:text-slate-400 disabled:cursor-not-allowed" />
                  </td>
                  <td class="p-1 border border-slate-200">
                    <input type="text" :disabled="!modalFormState.Code" :value="formatCurrencyInput(modalRateMatrix[getModalMatrixKey(rt.code, 'Triple')], selectedRateCode?.Currency)" @input="e => modalRateMatrix[getModalMatrixKey(rt.code, 'Triple')] = cleanCurrencyValue(e.target.value, selectedRateCode?.Currency)" class="w-full px-2 py-1.5 border border-slate-100 hover:border-slate-300 focus:border-sky-300 rounded text-center text-xs focus:outline-none font-semibold transition-colors disabled:bg-slate-50 disabled:text-slate-400 disabled:cursor-not-allowed" />
                  </td>
                  <td class="p-1 border border-slate-200">
                    <input type="text" :disabled="!modalFormState.Code" :value="formatCurrencyInput(modalRateMatrix[getModalMatrixKey(rt.code, 'Family')], selectedRateCode?.Currency)" @input="e => modalRateMatrix[getModalMatrixKey(rt.code, 'Family')] = cleanCurrencyValue(e.target.value, selectedRateCode?.Currency)" class="w-full px-2 py-1.5 border border-slate-100 hover:border-slate-300 focus:border-sky-300 rounded text-center text-xs focus:outline-none font-semibold transition-colors disabled:bg-slate-50 disabled:text-slate-400 disabled:cursor-not-allowed" />
                  </td>
                  <td class="p-1 border border-slate-200">
                    <input type="text" :disabled="!modalFormState.Code" :value="formatCurrencyInput(modalRateMatrix[getModalMatrixKey(rt.code, 'King')], selectedRateCode?.Currency)" @input="e => modalRateMatrix[getModalMatrixKey(rt.code, 'King')] = cleanCurrencyValue(e.target.value, selectedRateCode?.Currency)" class="w-full px-2 py-1.5 border border-slate-100 hover:border-slate-300 focus:border-sky-300 rounded text-center text-xs focus:outline-none font-semibold transition-colors disabled:bg-slate-50 disabled:text-slate-400 disabled:cursor-not-allowed" />
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div v-if="showCurrencyModal" class="fixed inset-0 z-[9999] flex items-center justify-center">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-[520px] max-h-[80vh] flex flex-col overflow-hidden">
          ...
          <div class="flex items-center justify-between px-5 py-3.5 bg-sky-500 text-white shrink-0">
            <span class="font-bold text-sm">Chọn tiền tệ</span>
            <button @click="showCurrencyModal = false; currencySearch = ''" class="w-7 h-7 flex items-center justify-center rounded-full hover:bg-white/20 transition-colors cursor-pointer bg-transparent border-none text-white">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
          <!-- Search -->
          <div class="px-5 py-3 border-b border-slate-100 shrink-0">
            <div class="relative">
              ...
              <input 
                ...
                placeholder="Search" 
                class="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-sky-400 bg-slate-50/50"
              />
            </div>
          </div>
          <!-- Currency List -->
          <div class="px-5 py-3 overflow-y-auto flex-1">
            <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-3">Tiền tệ phổ biến</p>
            <div class="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  <div class="text-sm font-bold text-slate-800 truncate">{{ cur.name }}</div>
                  <div class="text-xs text-slate-500 font-semibold">{{ cur.code }} - {{ cur.symbol }}</div>
                </div>
                <svg v-if="rateFormState.Currency === cur.code" class="w-5 h-5 text-emerald-500 shrink-0" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
                </svg>
              </button>
            </div>
            <p v-if="filteredCurrencies.length === 0" class="text-center text-slate-400 py-6 text-sm">Không tìm thấy tiền tệ phù hợp</p>
          </div>
        </div>
      </div>
    </Teleport>
