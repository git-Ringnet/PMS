...
class RoomMovesReportTest extends TestCase
{
    ...
    public function test_migration_preserves_legacy_room_move_filters_and_safe_sorting(): void
    {
        ...
        $this->assertStringContainsString('new_br.booking_id AS BookingCode1', $migration);
        $this->assertStringContainsString('MIN(guest_id) AS guest_id', $migration);
        $this->assertStringContainsString("DATE_FORMAT(old_br.actual_arrival_date, '%d-%m-%Y')", $migration);
        $this->assertStringContainsString("p_sort_by IN ('Room', 'Room1')", $migration);
        $this->assertStringNotContainsString('deleted_at IS NULL', $migration);
        ...
        $this->assertStringContainsString('registration-statuses,users', $controller);
    }
    ...
    public function test_reference_template_renders_legacy_room_move_columns(): void
    {
        ...
        $this->assertStringContainsString('1.000.000', $rendered);
        $this->assertStringContainsString('1.200.000', $rendered);
