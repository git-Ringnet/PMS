...
{
    private const CODE = 'BREAKAGE_INVOICES_BY_PRODUCT';
    private const PROCEDURE = 'rpt_breakage_invoices_by_product';
    private const TEMPLATE = 'BREAKAGE_INVOICES_BY_PRODUCT_STANDARD';
    ...
    public function up(): void
    {
        ...
        DB::table('report_data_sources')->updateOrInsert(['code'=>self::CODE], ['name'=>'Dữ liệu hóa đơn hàng bể vỡ theo sản phẩm','description'=>'Tổng hợp sản phẩm theo logic legacy sp_206 với freeitem=0.','source_type'=>'procedure','schema_name'=>$db,'object_name'=>self::PROCEDURE,'parameter_schema'=>json_encode($params, JSON_UNESCAPED_UNICODE),'field_schema'=>json_encode($fields, JSON_UNESCAPED_UNICODE),'sample_parameters'=>json_encode($defaults),'max_rows'=>5000,'is_active'=>true,'last_discovered_at'=>$now,'created_at'=>$now,'updated_at'=>$now]);
        $sourceId = DB::table('report_data_sources')->where('code',self::CODE)->value('id');
        $provider = require database_path('report_templates/BREAKAGE_INVOICES_BY_PRODUCT_reference.php'); $td = $provider->definition();
        ...
        DB::table('report_definitions')->updateOrInsert(['code'=>self::CODE], ['name'=>'Báo cáo hóa đơn hàng bể vỡ theo sản phẩm','group'=>'Báo cáo dịch vụ','description'=>'Tổng hợp sản phẩm hàng bể vỡ theo sp_206 legacy.','report_data_source_id'=>$sourceId,'parameter_ui_schema'=>json_encode($this->ui(),JSON_UNESCAPED_UNICODE),'sort_order'=>44,'is_active'=>true,'show_in_menu'=>true,'menu_locations'=>json_encode(['reservation','frontdesk']),'menu_top_order'=>40,'menu_group_order'=>20,'menu_item_order'=>44,'created_at'=>$now,'updated_at'=>$now]);
    ...
    {
        return <<<'SQL'
CREATE PROCEDURE `rpt_breakage_invoices_by_product`(IN p_from_date DATE,IN p_to_date DATE,IN p_shift VARCHAR(20),IN p_department VARCHAR(50),IN p_user VARCHAR(50),IN p_freeitem TINYINT,IN p_group_by_date TINYINT)
READS SQL DATA
BEGIN
 ...
 WHERE h.Outlet='BR' AND h.Date>=p_from_date AND h.Date<DATE_ADD(p_to_date,INTERVAL 1 DAY)
   AND h.Status=1
