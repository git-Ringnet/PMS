...
return new class extends Migration
{
    private const DEFINITION = [
        'outlet' => 'BR',
        'procedure' => 'rpt_breakage_free_invoices',
        'template' => 'BREAKAGE_FREE_INVOICES_STANDARD',
        'template_file' => 'breakage_free_invoices_reference.php',
        'name' => 'Báo cáo hóa đơn hàng bể vỡ miễn phí',
        'sort_order' => 45,
    ];
    ...
    public function up(): void
    {
        ...
        $installer->installReport('BREAKAGE_FREE_INVOICES', self::DEFINITION);
        if (DB::connection()->getDriverName() !== 'mysql') return;

        DB::unprepared('DROP PROCEDURE IF EXISTS `rpt_breakage_free_invoices`');
        DB::unprepared(<<<'SQL'
CREATE PROCEDURE `rpt_breakage_free_invoices`(
    IN p_from_date DATE,
    IN p_to_date DATE,
...
BEGIN
    ...
    WHERE h.Date >= p_from_date AND h.Date < DATE_ADD(p_to_date, INTERVAL 1 DAY)
      AND h.Outlet = 'BR' AND pay.PaymentMethod = 'CL'
      AND (p_user IS NULL OR p_user = '' OR h.BillUsername LIKE CONCAT('%', p_user, '%'))
    ORDER BY STT;
END
SQL);

        DB::table('report_data_sources')->where('code', 'BREAKAGE_FREE_INVOICES')->update([
        ...
        $source = DB::table('report_data_sources')->where('code', 'BREAKAGE_FREE_INVOICES')->first();
