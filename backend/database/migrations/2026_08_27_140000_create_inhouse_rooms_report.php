...
BEGIN
    ...
    WITH RECURSIVE report_dates AS (
        ...
        UNION ALL SELECT DATE_ADD(stay_date, INTERVAL 1 DAY) FROM report_dates WHERE stay_date < p_to_date
    ),
    room_days AS (
        SELECT br.id, d.stay_date,
               COALESCE(rnb.rate, br.rate) AS night_rate,
               CASE WHEN EXISTS (
                   SELECT 1 FROM service_bills sb INNER JOIN room_night_bills rnb2 ON rnb2.bill_id = sb.Ma
                   WHERE sb.RentalRoomId1 = br.id AND DATE(sb.Date) = d.stay_date AND rnb2.is_room_night = 1
               ) THEN 1 ELSE 0 END AS has_room_night
        FROM booking_rooms br CROSS JOIN report_dates d
    ...
    eligible AS (
        ...
        WHERE br.deleted_at IS NULL
          ...
          AND ((p_vat = 1 AND p_no_vat = 0 AND b.has_vat = 1) OR (p_vat = 0 AND p_no_vat = 1 AND b.has_vat = 0) OR (p_vat = 0 AND p_no_vat = 0))
          AND (
             (p_actual = 1 AND rd.stay_date = v_system_date AND br.status = 1 AND (br.departure_date >= v_system_date OR br.is_day_use = 1 OR b.is_day_use = 1))
             OR (p_actual = 0 AND rd.stay_date = v_system_date AND ((br.status = 1 AND br.departure_date > v_system_date) OR (br.status = 0 AND br.arrival_date = v_system_date)))
             OR (rd.stay_date <> v_system_date AND rd.stay_date >= br.arrival_date AND rd.stay_date < br.departure_date AND rd.has_room_night = 1)
             OR (br.status = 4 AND rd.has_room_night = 1)
          )
    )
    SELECT result.*, ROUND(100 * COUNT(*) OVER (PARTITION BY result.SummaryRoomTypeCode) / NULLIF(COUNT(*) OVER (), 0), 2) AS RoomTypePercent
    FROM (
    SELECT e.stay_date AS StayDateGroup,
           ...
           CONCAT(DATE_FORMAT(e.CheckoutDate, '%d/%m/%Y'), IF(e.CheckoutDate IS NULL, '', '')) AS ActualDepartureDate,
           ...
           COALESCE(e.room_note, e.booking_note) AS Note, e.company_id AS CompanyId, e.company_name AS Company,
           e.registration_status_name AS BookingStatusName, g.is_primary AS IsMainGuest,
           e.room_type_code AS SummaryRoomTypeCode, e.room_order AS RoomOrder
    FROM eligible e
    ...
    SELECT e.stay_date, CONCAT(e.booking_id, ' - ', e.booking_name), e.booking_id, e.booking_id, e.id,
           ...
           NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, e.company_id, e.company_name,
           NULL, -1, e.room_type_code, e.room_order
    FROM eligible e
