...
SQL;
    ...
    private function seedConfiguration(): void
    {
        ...
        DB::table('templates')->updateOrInsert(['report'=>self::TEMPLATE], ['group'=>'Báo cáo thống kê lễ tân','name'=>'Báo cáo tiền đặt cọc','report_data_source_id'=>$sourceId,'parameter_defaults'=>json_encode($defaults),'page_size'=>$definition['page_size'],'page_orientation'=>$definition['page_orientation'],'margin_top'=>$definition['margin_top'],'margin_right'=>$definition['margin_right'],'margin_bottom'=>$definition['margin_bottom'],'margin_left'=>$definition['margin_left'],'content_json'=>json_encode($definition['content_json'],JSON_UNESCAPED_UNICODE),'content_html'=>$definition['content_html'],'css'=>$definition['css'],'is_default'=>false,'version'=>'1.0','created_at'=>$now,'updated_at'=>$now]);
        ...
        DB::table('report_definitions')->updateOrInsert(['code'=>self::REPORT], ['name'=>'Báo cáo đặt cọc (Module Sale)','group'=>'Báo cáo thống kê lễ tân','description'=>'Báo cáo đặt cọc theo ngày payment.date, bộ phận, user, ca, công ty và phương thức thanh toán.','report_data_source_id'=>$sourceId,'parameter_ui_schema'=>json_encode($ui,JSON_UNESCAPED_UNICODE),'sort_order'=>39,'is_active'=>true,'show_in_menu'=>true,'menu_locations'=>json_encode(['reservation','frontdesk']),'menu_top_order'=>20,'menu_group_order'=>30,'menu_item_order'=>39,'created_at'=>$now,'updated_at'=>$now]);
