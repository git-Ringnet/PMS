...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        $db->unprepared($this->procedureSql(usePerformerCode: true));
    ...
    {
        // Keep the AC exclusion and current allocation layout when rolling back unrelated schema work.
    }

    private function procedureSql(bool $usePerformerCode): string
    {
        $usernameSelect = $usePerformerCode
            ? "COALESCE(NULLIF(p.created_by, ''), NULLIF(p.username, ''), '') AS Username"
            : 'p.username AS Username';
        $usernameFilter = $usePerformerCode
            ? "COALESCE(NULLIF(p.created_by, ''), NULLIF(p.username, ''), '') = p_user"
            : 'p.username = p_user';
...
BEGIN
    WITH base AS (
        ...
        WHERE p.deleted_at IS NULL
          ...
          AND (COALESCE(p_user, '') = '' OR {$usernameFilter})
