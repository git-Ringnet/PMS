...
{
    private const CODE = 'MINIBAR_INVOICES_BY_PRODUCT';
    private const PROCEDURE = 'rpt_minibar_invoices_by_product';
    private const TEMPLATE = 'MINIBAR_INVOICES_BY_PRODUCT_STANDARD';
    ...
    public function up(): void
    {
        ...
        DB::table('report_data_sources')->updateOrInsert(
            ['code' => self::CODE],
            [
                'name' => 'Dữ liệu hóa đơn minibar theo sản phẩm',
                'description' => 'Tổng hợp sản phẩm minibar theo logic legacy sp_206 với freeitem=0.',
        ...
        DB::table('templates')->updateOrInsert(
            ...
            [
                ...
                'page_size' => $td['page_size'],
                'page_orientation' => $td['page_orientation'],
                'margin_top' => $td['margin_top'],
                'margin_bottom' => $td['margin_bottom'],
                'margin_left' => $td['margin_left'],
                'margin_right' => $td['margin_right'],
                'content_json' => json_encode($td['content_json'], JSON_UNESCAPED_UNICODE),
                'content_html' => $td['content_html'],
                'css' => $td['css'],
                'is_default' => false,
                'version' => $td['version'],
                'created_at' => $now,
        ...
        DB::table('report_definitions')->updateOrInsert(
            ['code' => self::CODE],
            [
                'name' => 'Báo cáo hóa đơn minibar (theo sản phẩm)',
                'group' => 'Báo cáo dịch vụ',
                'description' => 'Tổng hợp sản phẩm minibar theo sp_206 legacy.',
                'report_data_source_id' => $sourceId,
                'parameter_ui_schema' => json_encode($this->ui(), JSON_UNESCAPED_UNICODE),
                'sort_order' => 43,
                'is_active' => true,
                'show_in_menu' => true,
                'menu_locations' => json_encode(['reservation', 'frontdesk', 'housekeeping']),
                'menu_top_order' => 40,
                'menu_group_order' => 20,
                'menu_item_order' => 43,
                'created_at' => $now,
    ...
    {
        return <<<'SQL'
CREATE PROCEDURE `rpt_minibar_invoices_by_product`(
    IN p_from_date DATE,
    IN p_to_date DATE,
    IN p_shift VARCHAR(20),
    IN p_department VARCHAR(50),
    IN p_user VARCHAR(50),
    IN p_freeitem TINYINT,
    IN p_group_by_date TINYINT
)
READS SQL DATA
BEGIN
    SELECT
        ROW_NUMBER() OVER (ORDER BY CASE WHEN COALESCE(p_group_by_date, 0) = 1 THEN sub.DateGroup ELSE NULL END ASC, sub.ProductType ASC, sub.Product ASC) AS ID,
        sub.DateGroup,
        sub.ProductType,
        sub.MaProduct,
        sub.Product,
    ...
    FROM (
        SELECT
            CASE WHEN COALESCE(p_group_by_date, 0) = 1 THEN DATE_FORMAT(MIN(h.Date), '%d/%m/%Y') ELSE NULL END AS DateGroup,
            COALESCE(MIN(c.name), 'Minibar') AS ProductType,
            MIN(d.MaProduct) AS MaProduct,
            MIN(d.Product) AS Product,
            COALESCE(MIN(h.Currency), '') AS Currency,
        ...
        LEFT JOIN product_categories c ON c.id = d.ProductGroupId
        WHERE h.Outlet = 'MB'
          ...
          AND d.Deleted = 0
        GROUP BY
            CASE WHEN COALESCE(p_group_by_date, 0) = 1 THEN DATE(h.Date) ELSE NULL END,
            COALESCE(c.name, 'Minibar'),
            d.MaProduct,
            d.Product
        HAVING SUM(d.Quantity) > 0
    ) sub
    ORDER BY CASE WHEN COALESCE(p_group_by_date, 0) = 1 THEN sub.DateGroup ELSE NULL END ASC, sub.ProductType ASC, sub.Product ASC;
END
SQL;
    }
    ...
    private function parameters(): array
    {
        return [
            ...
            ['name' => 'p_freeitem', 'mode' => 'IN', 'data_type' => 'tinyint', 'database_type' => 'tinyint', 'position' => 6, 'required' => false],
            ['name' => 'p_group_by_date', 'mode' => 'IN', 'data_type' => 'tinyint', 'database_type' => 'tinyint', 'position' => 7, 'required' => false],
        ];
    }
    ...
    private function fields(): array
    {
        return array_map(
            fn ($n) => [
                'name' => $n,
                'type' => in_array($n, ['ID', 'MaProduct', 'Quantity', 'Rate', 'Amount', 'DiscountAmount', 'Total'], true) ? 'number' : 'string',
                'nullable' => true,
            ],
            ['ID', 'DateGroup', 'ProductType', 'MaProduct', 'Product', 'Currency', 'Quantity', 'Rate', 'Amount', 'DiscountAmount', 'Total']
        );
    }
    ...
    private function ui(): array
    {
        return [
            ...
            ['name' => 'p_shift', 'label' => 'Ca làm việc', 'control' => 'select', 'default' => '', 'required' => false, 'placeholder' => 'Select Value', 'options_source' => 'report-shifts', 'options' => []],
            ['name' => 'p_department', 'label' => 'Chọn bộ phận', 'control' => 'select', 'default' => '', 'required' => false, 'placeholder' => 'Select Value', 'options_source' => 'service-departments', 'options' => []],
            ...
            ['name' => 'p_freeitem', 'label' => 'Hàng bán', 'control' => 'checkbox', 'default' => 0, 'required' => false],
    ...
    public function down(): void
    {
        ...
        $id = DB::table('report_definitions')->where('code', self::CODE)->value('id');
        $tid = DB::table('templates')->where('report', self::TEMPLATE)->value('id');

        if ($id) {
            DB::table('report_definition_template')->where('report_definition_id', $id)->delete();
            DB::table('report_definitions')->where('id', $id)->delete();
        }
        if ($tid) {
            DB::table('templates')->where('id', $tid)->delete();
        }

        DB::table('report_data_sources')->where('code', self::CODE)->delete();
        DB::unprepared('DROP PROCEDURE IF EXISTS `'.self::PROCEDURE.'`');
    }
};
