...
begin
	...
	insert into #TempResult values ('1-1', N'Tổng doanh thu',(select Round(isnull(sum(Amount),0), 0) from #tempRev where Date = @date),(select isnull(sum(Amount), 0) from #tempRev), 1)



	insert into #TempResult values ('1-2', N'Doanh thu phòng',(select Round(isnull(sum(Amount),0), 0) from #tempRev where Date = @date and ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'RoomRevenue'),','))
),(select isnull(sum(Amount), 0) from #tempRev where ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'RoomRevenue'),','))), 0)



	insert into #TempResult values ('1-3', N'Doanh thu nhà hàng',(select Round(isnull(sum(Amount),0), 0) from #tempRev where Date = @date and ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'FbRevenue'),',')
)),(select isnull(sum(Amount), 0) from #tempRev where ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'FbRevenue'),','))), 0)



	insert into #TempResult values ('1-4', N'Doanh thu hội nghị',(select Round(isnull(sum(Amount),0), 0) from #tempRev where Date = @date and ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'ConferenceRevenu
e'),','))),(select isnull(sum(Amount), 0) from #tempRev where ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'ConferenceRevenue'),','))), 0) 



	insert into #TempResult values ('1-5', N'Doanh thu Minibar',(select Round(isnull(sum(Amount),0), 0) from #tempRev where Date = @date and ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'MinibarRevenue'),
','))),(select isnull(sum(Amount), 0) from #tempRev where ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'MinibarRevenue'),','))), 0)
	...
	insert into #TempResult values ('1-7', N'Doanh thu giặt ủi',(select Round(isnull(sum(Amount),0), 0) from #tempRev where Date = @date and ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'LaundryRevenue'),
','))),(select isnull(sum(Amount), 0) from #tempRev where ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'LaundryRevenue'),','))), 0)
	...
	insert into #TempResult values ('1-9', N'Doanh thu vận chuyển',(select Round(isnull(sum(Amount),0), 0) from #tempRev where Date = @date and ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'Transportation
Revenue'),','))),(select isnull(sum(Amount), 0) from #tempRev where ServiceId in (select value from string_split((select [Service] from #tempSetup where DisplayName = 'TransportationRevenue'),','))), 0)



	insert into #TempResult values ('1-10', N'Doanh thu dịch vụ khác',(select Round(isnull(sum(Amount),0), 0) from #tempRev where Date = @date and ServiceId in (select Ma from sp1306 where ma not in(select value from string_split((select string_agg(service, 
',') from sp1610 where Report = 'NightAuditReport'), ',')))),(select isnull(sum(Amount), 0) from #tempRev where ServiceId in (select Ma from sp1306 where ma not in(select value from string_split((select string_agg(service, ',') from sp1610 where Report = 
'NightAuditReport'), ',')))), 0)
