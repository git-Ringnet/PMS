...
begin
	...
	select dk.Status as BookingStatus, pt.Status as StatusRoom, dv.InvoiceId as BillID, bcdt.Date, isnull(bcdt.Room, tmp.Room)  Room, bcdt.Guest, bcdt.OpenTime, bcdt.PaymentID, bcdt.Amount, bcdt.Username, bcdt.Description, bcdt.PaymentMethod, bcdt.Department, pt.Room  as NumOfRoom, case when bcdt.Amount < 0 then 2 else 
	...
	case when isnull(bcdt.Pack2, '') = '' then N'Thu Ngƒn' else N'D?t c?c' end end as ShowDeposit,
		...
		case when bcdt.RegisterID2 is null then concat(k.Title , ' ' , k.FirstName) else dk.BookingName end GuestInfo,dk.Provide2,ct.Company, case when bcdt.paymentmethod = 'CD' then replicate('*', len(card.CardId) -4) + RIGHT (card.CardId, 4) else '' end as CardId, @division as Division
	from vw_004 bcdt 
		...
		left join #tempKhachLe tmp on tmp.BookingId = dk.Ma
