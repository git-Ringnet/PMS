...
-- exec sp_240 '20241201','20241211',0, '00:00', '23:59'

CREATE   proc [dbo].[sp_240](@fromDate date, @toDate date, @companyId int, @fromTime varchar(5), @toTime varchar(5))
 as
begin
...
from sp2000 bk
left join sp2100 pt on bk.ma = pt.BookingId
group by bk.ma
...
if(@template = 'Tulip')
	begin
		insert into #tempHD
		select ( @division + cast(isnull(hd.RegisterId1, pt.BookingId) as varchar(20))) as CodeBooking, isnull(pt.Status, dk.Status) as Status,pt.Ma as RoomCode,
		isnull(pt.Room,'') as RoomNumber,
		ct.Company as Company,
		case when hd.CustomerId2 is null then isnull((select top 1 FirstName from SP2200 left join SP2300 on Id = CustomerId where RentalRoomId = pt.Ma and IsMainGuest = 1 ),dk.BookingName) else k.FirstName end as GuestName,
		...
		isnull(pt.Room,'') as RoomNumber,
		ct.Company as Company,
		case when hd.CustomerId2 is null then isnull((select FirstName from SP2200 left join SP2300 on Id = CustomerId where RentalRoomId =  pt.Ma and IsMainGuest = 1 ),dk.BookingName) else k.FirstName end as GuestName,
		--dk.BookingName as GuestName,
		...
		left join SP2300 k on k.Id = hd.CustomerId2
		left join SP2000 dk on dk.Ma = isnull(hd.RegisterID2,pt.BookingId)
		left join #tempDateBooking t on t.Ma = dk.Ma
		left join SP1302 ct on dk.TravelAgency = ct.Ma
		where isnull(hd.Edit,0) = 0 and hd.ServiceId != 'RM' and (tt.PaymentMethod is null or tt.PaymentMethod not in (select Ma from sp1326 where HTMienPhi = 1))
		and (@companyId = '' or ct.Ma = @companyId)
	...
	begin
		...
		isnull( pt.Room,'') as RoomNumber,
		ct.Company as Company,
		--case when CustomerId2 is null then isnull((select FirstName from SP2200 left join SP2300 on Id = CustomerId where RentalRoomId = pt.Ma and IsMainGuest = 1 ),dk.BookingName) else k.FirstName end as GuestName,
		dk.BookingName as GuestName,
		...
		isnull(hd.ServiceId,'') as ServiceIdHD, 
		isnull( pt.ArrivalDate,dk.ArrivalDate) as Arrival,
		isnull( pt.CheckoutDate,dk.ArrivalDate + dk.NumOfDays) as Departure, 
		isnull( pt.CheckoutTime, cast(t.DateBooking as time)) as CheckoutTime,
		isnull(hdct.DepartmentId,'') as DepartmentId, 
		isnull(hd.Outlet,'') as Outlet, isnull(CustomerId2,cast(isnull(cast(RegisterID2 as varchar), pt.Ma) as varchar)) as MaChung,
		pt.DayUse, 0
		from SP3000 hd
		join SP3001 hdct on hdct.BillServiceId = hd.Ma
		full outer join SP2100 pt on pt.Ma = hd.RentalRoomId2
		left join SP2300 k on k.Id = hd.CustomerId2
		...
		group by  hdct.ServiceId,hd.RegisterID2 ,hd.RentalRoomId2, pt.BookingId, pt.Room,
					ct.Company, k.FirstName, dk.BookingName, pt.ArrivalDate, pt.CheckoutDate ,hd.ServiceId,
					hdct.DepartmentId, hd.Outlet, 
					dk.ArrivalDate, dk.NumOfDays, pt.DayUse,hd.CustomerId2, pt.Ma,pt.Status,dk.Status, pt.CheckoutTime, t.DateBooking
		order by  pt.CheckoutDate, CodeBooking

	end

	
select CodeBooking, RoomCode,Status, RoomNumber, Company, GuestName, Arrival, Departure, 
...
case when @template = 'Tulip' then 0 else isnull((select SUM(Amount) from SP3002 where PaymentMethod = 'BT' and isnull(CustomerId2,cast(isnull(cast(RegisterID2 as varchar),CustomerId2) as varchar)) = tt.MaChung),0) end as BankTransfer,
case when @template = 'Tulip' then 0 else isnull((select SUM(Amount) from SP3002 where PaymentMethod = 'CD' and isnull(CustomerId2,cast(isnull(cast(RegisterID2 as varchar),CustomerId2) as varchar)) = tt.MaChung),0) end as CreditCard,			  
case when @template = 'Tulip' then 0 else isnull((select SUM(Amount) from SP3002 where PaymentMethod = 'CA' and isnull(CustomerId2,cast(isnull(cast(RegisterID2 as varchar),CustomerId2) as varchar)) = tt.MaChung),0) end as Cash,
case when @template = 'Tulip' then 0 else isnull((select SUM(Amount) from SP3002 where PaymentMethod = 'AC' and isnull(CustomerId2,cast(isnull(cast(RegisterID2 as varchar),CustomerId2) as varchar)) = tt.MaChung),0) end as CityLedger,			
case when @template = 'Tulip' then 0 else isnull((select SUM(Amount) from SP3002 where PaymentMethod not in ('AC','BT','CD','CA','BC') and ISNULL(RentalRoomId2,cast(RegisterID2 as varchar)) = tt.MaChung),0) end as OtherPayment,
isnull((select STRING_AGG(PaymentMethod,',') from SP3002  where Edit = 0 and PaymentID = tt.PaymentID),'') as PaymentMethod,
case when @template = 'Tulip' then 0 else isnull((select SUM(Amount) from SP3002 where PaymentMethod = 'BC' and isnull(CustomerId2,cast(isnull(cast(RegisterID2 as varchar),CustomerId2) as varchar)) = tt.MaChung),0) end as BankTransferTKCN


into #tempTotal
from #tempHD tt
where cast(concat(DepartureBK, ' ',isnull(CheckoutTime,@fromTime)) as datetime) between cast(concat(@fromDate, ' ',@fromTime) as datetime) and cast(concat(@toDate, ' ',@toTime) as datetime) --and (RoomNumber like '0%' or status in (2,100))----and status != 1

--select * from #tempHD

select CodeBooking,isnull(RoomCode,'') as  RoomCode,Status, RoomNumber , Company, GuestName, Arrival, Departure, 
...
STRING_AGG(PaymentMethod,',') as PaymentMethod

into #tempResult
from #tempTotal
group by CodeBooking, RoomCode,Status, RoomNumber, Company, GuestName, Arrival, Departure

update #tempResult set PaymentMethod = (select string_agg(A.Data,',') from (select distinct data from dbo.func_061(#tempResult.PaymentMethod,',') where Data !='') A)


update #tempResult set OtherRevenue = TotalRevenue - ([RoomCharge]/*+[DayUse]*/+[ExtraBed]+[ExtraPerson]/*+[SofaBed]*/+[ExtraRoomCharge]+[Tour]+[Transportation]+[Miscel]+[Minibar]+[Laundry]+[Broken]+[Breakfast Splitdown]+[Breakfast Charge]+[Food_Res]+[Beverage_Res]/*+[Set Menu]*/+[Other_Res]+[Food_Room]+[Beverage_Room]+[Other_Room]+[Food_Meet]+[Beverage_Meet]+[Other_Meet]+[Mice]+[Other])


select * from #tempResult  order by case when RoomNumber like '0%' then 0 else replace(CodeBooking, @division,'') end, RoomNumber

drop table #tempHD
drop table #tempTotal
drop table #tempResult
drop table #tempDateBooking
