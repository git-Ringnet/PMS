...
RETURNS @re TABLE (
    ...
    -- CH?T CH?N V?T LY: Gi�p c�c c�u Join/Update sau n�y nhanh g?p 100 l?n
...
BEGIN
    ...
    -- SECTION 1: Historical (fDate1..tDate1) - Gi? nguy�n Set-based g?c
    ...
    BEGIN
        ...
        SELECT ISNULL(pt.Ma, pt2.Ma), CASE WHEN pt.Ma IS NOT NULL THEN pt.BookingId ELSE dk.Ma END, h.Ma, h.ServiceId, h.Date, TotalAmount0, hdtp.RateCode, ISNULL(dk.WalkIn,1), h.DepartmentId, h.Quantity, ISNULL(ptdvtd.Promotion, ISNULL(pt.Pack3, pt2.Pack
3))
                                                                                                                                                                                                                                                          
        FROM SP3004 hdtp LEFT JOIN SP3000 h ON hdtp.BillId = h.Ma LEFT JOIN SP2100 pt2 ON pt2.Ma = h.RentalRoomId2 LEFT JOIN SP2100 pt ON pt.Ma = h.RentalRoomId1 LEFT JOIN SP2000 dk ON dk.Ma = h.RegisterID2 LEFT JOIN Sp2102 ptdvtd ON ptdvtd.RentalRoomId =
 ISNULL(pt.Ma, pt2.Ma) AND ptdvtd.FromDate = h.Date AND ptdvtd.ServiceId = 'RM'
        ...
        SELECT DISTINCT CASE WHEN pt.Ma IS NOT NULL THEN pt.Ma ELSE pt2.Ma END, CASE WHEN pt.Ma IS NOT NULL THEN pt.BookingId ELSE dk.Ma END, h.Ma, h.ServiceId, h.Date, TotalAmount0, CASE WHEN pt.Ma IS NOT NULL THEN pt.RoomRateCode ELSE pt2.RoomRateCode E
ND, ISNULL(dk.WalkIn,1), h.DepartmentId, h.Quantity
                                                                                                                                                                                                          
        FROM SP3000 h LEFT JOIN SP2100 pt2 ON pt2.Ma = h.RentalRoomId2 LEFT JOIN SP2100 pt ON pt.Ma = h.RentalRoomId1 LEFT JOIN SP2000 dk ON dk.Ma = h.RegisterID2 INNER JOIN SP2102 ptdvtd ON ptdvtd.RentalRoomId = CASE WHEN pt.Ma IS NOT NULL THEN pt.Ma ELS
E pt2.Ma END AND ptdvtd.FromDate = h.Date AND ptdvtd.ServiceId = h.ServiceId
                                                                                                                                                                                 
        WHERE h.Edit = 0 AND h.Date BETWEEN @fDate1 AND @tDate1 AND h.ServiceId <> 'RM' AND (@registrationID = '' OR CASE WHEN pt.Ma IS NOT NULL THEN pt.BookingId ELSE dk.Ma END IN (SELECT Id FROM @RegIds)) AND h.RentalRoomId1 IN (SELECT pt.Ma FROM SP2100
 pt WHERE pt.Status IN (2,4,100) AND (pt.Room IS NULL OR pt.Room NOT LIKE '0%%'));
...
E pt2.Ma END AND ptdvtd.FromDate = h.Date AND ptdvtd.ServiceId = h.ServiceId
        ...
        WHERE h.Edit = 0 AND h.Date BETWEEN @fDate1 AND @tDate1 AND h.ServiceId <> 'RM' AND (@registrationID = '' OR CASE WHEN pt.Ma IS NOT NULL THEN pt.BookingId ELSE dk.Ma END IN (SELECT Id FROM @RegIds)) AND h.RentalRoomId1 IN (SELECT pt.Ma FROM SP2100
 pt WHERE pt.Status IN (0,1) AND (pt.Room IS NULL OR pt.Room NOT LIKE '0%%'));
        ...
        WHERE h.Edit = 0 AND h.Date BETWEEN @fDate1 AND @tDate1 AND h.ServiceId <> 'RM' AND h.RentalRoomId1 IN (SELECT ma FROM SP2100 WHERE Room LIKE '0%') AND (@registrationID = '' OR CASE WHEN pt.Ma IS NOT NULL THEN pt.BookingId ELSE '' END IN (SELECT I
d FROM @RegIds));
   ...
   -- =========================================================
    ...
    BEGIN
        ...
        DECLARE @Bookings TABLE (
            ...
            Ma varchar(10) PRIMARY KEY CLUSTERED, -- B?t bu?c Ma l� h?t duy nh?t
                                                                                                                                                                             
            BookingId bigint, ArrivalDate date, DepartureDate date, Rate decimal(18,2), ActualNumOfDays int, DayUse varchar(10), RoomType smallint, RoomKind smallint, Pack2 varchar(20), Pack3 nvarchar(500), RoomRateCode varchar(20), VoucherInfo varchar(ma
x), ExtraBed int, ExtraBedRate money, Child int, ActualArrivalDate date
        ...
        FROM SP2100 pt LEFT JOIN SP2000 dk ON dk.Ma = pt.BookingId LEFT JOIN @LateIn li ON pt.Ma = li.RentalRoomId1 AND li.LateCheckInDate < pt.ArrivalDate LEFT JOIN @ServiceMax sm ON pt.Ma = sm.RentalRoomId1 AND sm.ServiceDate > DATEADD(day, CASE WHEN pt
.ActualNumOfDays=0 THEN 0 ELSE pt.ActualNumOfDays-1 END, ISNULL(li.LateCheckInDate, pt.ArrivalDate))
        ...
        WHERE pt.Status IN (0,1,2,4,100) AND (pt.Room IS NULL OR pt.Room NOT LIKE '0%%') AND (@registrationID = '' OR pt.BookingId IN (SELECT Id FROM @RegIds))
          ...
          AND (DATEDIFF(day, ISNULL(li.LateCheckInDate, pt.ArrivalDate), ISNULL(sm.ServiceDate, DATEADD(day, pt.ActualNumOfDays, pt.ArrivalDate))) != 0 OR EXISTS (SELECT 1 FROM SP3000 h2 INNER JOIN SP3004 hd2 ON hd2.BillId = h2.Ma AND hd2.IsRoomNight = 1 
WHERE h2.RentalRoomId1 = pt.Ma AND h2.Date = pt.ArrivalDate) OR pt.DayUse = '1');
        ...
        WHERE h.Edit = 0 AND h.Date BETWEEN @fDate2 AND @tDate2 AND h.ServiceId = 'RM' AND (@registrationID = '' OR CASE WHEN pt.Ma IS NOT NULL THEN pt.BookingId ELSE '' END IN (SELECT Id FROM @RegIds)) AND h.RentalRoomId1 IN (SELECT ma FROM SP2100 WHERE 
Room LIKE '0%');
                                                                                                                                                                                                                                             

                                                                                                                                                                                                                                                             
        -- 3 B?NG CACHE KEM QUANTITY C?A 2102
        ...
        INSERT INTO @Cache77 SELECT b.Ma, f.[Date], f.Rate, f.PackageCode, f.ServiceId, f.DepartmentId, f.Quantity FROM @Bookings b CROSS APPLY dbo.func_077(b.Ma, b.ArrivalDate, b.DepartureDate, b.Pack2, b.RoomType, b.RoomKind) f WHERE f.[Date] BETWEEN @f
Date2 AND @tDate2;
                                                                                                                                                                                                                                           

                                                                                                                                                                                                                                                             
        DECLARE @Cache2102 TABLE (RentalRoomId varchar(10), ServiceId varchar(10), FromDate date, ToDate date, Total decimal(18,2), RoomRateCode varchar(30), IsRoom smallint, Ma int, Promotion varchar(500), Quantity float, INDEX IX CLUSTERED (RentalRoomId
, ServiceId, FromDate, ToDate));
        ...
        DECLARE @MasterGrid TABLE (Ma varchar(10), BookingId bigint, [Date] date, RoomRateCode varchar(20), ExtraBed int, ExtraBedRate money, Child int, ActualArrivalDate date, ActualNumOfDays int, DayUse varchar(10), Total decimal(18,2), Pack3 nvarchar(5
00), INDEX IX CLUSTERED (Ma, [Date]));
        ...
        -- S?A CH?A L?N: BOC VOUCHER VA PACK3 L?Y "MA PHONG" (Ma) LAM G?C B?N L?
        ...
        FROM @Bookings b CROSS APPLY OPENJSON(b.VoucherInfo) WITH (VoucherCode varchar(50) '$.VoucherCode', DiscountType int '$.DiscountType', VoucherAmount float '$.VoucherAmount', ServiceId varchar(10) '$.ServiceId', ListDays nvarchar(max) '$.ListDays' 
AS JSON) j CROSS APPLY OPENJSON(j.ListDays) WITH (Value varchar(50) '$.Value', Amount decimal '$.Amount') d
                                                                                                                                                  
        WHERE b.VoucherInfo IS NOT NULL AND SUBSTRING(d.Value, 0, CHARINDEX('|', d.Value, 0)) = b.Ma;
                                                                                                                                                        

                                                                                                                                                                                                                                                             
        -- Kh�a ch�nh b�y gi? l� Ma Ph�ng thu� (Ma)
        ...
        -- THE BIG RM INSERT (Join qua p3.Ma v� v.Ma)
        ...
        LEFT JOIN @ParsedPack3 p3 ON p3.Ma = mg.Ma -- <--- S?A JOIN THEO PHONG
                                                                                                                                                                               
        LEFT JOIN @ParsedVouchers v ON v.Ma = mg.Ma AND v.ServiceId = 'RM' AND v.TargetDate = mg.[Date] -- <--- S?A JOIN THEO PHONG
        ...
        CROSS APPLY (
            ...
            SELECT PromoRate = CASE 
                ...
                WHEN c2102.Promotion IS NOT NULL THEN 
                    ...
                    CASE WHEN JSON_VALUE(c2102.Promotion,'$.ValueType')='percent' THEN R1.RawRate * (1.0 + (CASE WHEN JSON_VALUE(c2102.Promotion,'$.Type')='discount' THEN -1.0 ELSE 1.0 END) * (CASE WHEN CAST(JSON_VALUE(c2102.Promotion,'$.PromotionValue') 
AS float) BETWEEN 0 AND 100 THEN CAST(JSON_VALUE(c2102.Promotion,'$.PromotionValue') AS float) ELSE 0.0 END)/100.0)
                                                                                                                                          
                         ELSE CASE WHEN JSON_VALUE(c2102.Promotion,'$.Type')='discount' AND R1.RawRate < CAST(JSON_VALUE(c2102.Promotion,'$.PromotionValue') AS float) THEN 0.0 ELSE R1.RawRate + (CASE WHEN JSON_VALUE(c2102.Promotion,'$.Type')='discount' TH
EN -1.0 ELSE 1.0 END) * CAST(JSON_VALUE(c2102.Promotion,'$.PromotionValue') AS float) END END
                                                                                                                                                                
                ELSE 
                                                                                                                                                                                                                                        
                    CASE WHEN ISNULL(p3.ValueType, 'percent')='percent' THEN R1.RawRate * (1.0 + (CASE WHEN ISNULL(p3.[Type],'surcharge')='discount' THEN -1.0 ELSE 1.0 END) * (CASE WHEN ISNULL(p3.PromotionValue,0) BETWEEN 0 AND 100 THEN ISNULL(p3.Promotio
nValue,0) ELSE 0.0 END)/100.0)
                                                                                                                                                                                                                               
                         ELSE CASE WHEN (CASE WHEN ISNULL(p3.[Type],'surcharge')='discount' THEN -1.0 ELSE 1.0 END)=-1.0 AND R1.RawRate < ISNULL(p3.PromotionValue,0) THEN 0.0 ELSE R1.RawRate + (CASE WHEN ISNULL(p3.[Type],'surcharge')='discount' THEN -1.0 
ELSE 1.0 END) * ISNULL(p3.PromotionValue,0) END END 
        ...
        FROM @MasterGrid mg JOIN (SELECT mgp.PackageCode, mgp.ServiceCode, mgp.OrderDay, ISNULL(mgp.Total,0) AS Total, pkg.BeginDate, pkg.EndDate, mgp.Quantity FROM SP1317 mgp LEFT JOIN SP1323 pkg ON mgp.PackageCode=pkg.Ma WHERE mgp.Status<>3 AND mgp.Serv
iceCode<>'RM') pd ON mg.RoomRateCode = pd.PackageCode AND DATEDIFF(Day, mg.ActualArrivalDate, mg.[Date]) = pd.OrderDay - 1 WHERE mg.[Date] BETWEEN pd.BeginDate AND pd.EndDate;
                                                                              

                                                                                                                                                                                                                                                             
        -- Update Non-RM Vouchers (Join kh?p ch�nh x�c qua RentalRoomId = v.Ma)
