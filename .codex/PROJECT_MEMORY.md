...
- 2026-09-25: Checkout dòng 233: khi bill dịch vụ được cọc/tạm ứng hợp lệ bù đủ, người dùng bấm **Thêm** để thêm payment 0đ vào bảng rồi lưu; API xác thực lại bill/cọc cùng phạm vi và không tạo invoice rỗng. `SalesInvoiceSettlementTest` đạt 8/8 (61 assertions), PHP lint và frontend production build đạt; chưa nghiệm thu trên browser với dữ liệu thật. Không đổi schema hoặc luồng thanh toán khác.
- 2026-09-25: Checkout dòng 292: Payment Modal tự điền số còn phải thu vào ô nhập sau khi thêm/xóa dòng thanh toán; không đổi cách Lưu, tổng kiểm tra, API hoặc DB. Frontend production build đạt; chưa kiểm tra trực tiếp trên browser. Dòng 242 chưa triển khai.
- 2026-09-25: Tách No Post Master/phòng: API Master chỉ cập nhật `bookings.no_post`, không cascade; Master post tiền phòng bỏ qua phòng No Post riêng lẻ và trả danh sách cho UI; Checkout và điều chỉnh giá chỉ khóa Booking/phòng đích tương ứng. Giữ nguyên dữ liệu hiện có, không migration/backfill. Build và PHP lint đạt; 4 test No Post mục tiêu đạt. Toàn bộ `BookingRoomServiceFolioTest` đạt 24/28; 4 test lưu dịch vụ/khách/thuế còn lỗi.
- 2026-09-25: Bổ sung pre-check No Post tại UI Checkout/Hóa đơn và Buồng phòng: khóa nút/form FO/HK/tiền phòng, chặn thêm vào giỏ và gửi bill; đánh dấu phòng No Post trong bộ chọn Buồng phòng; khóa phòng đích điều chỉnh giá. Không đổi API/backend hoặc các màn hình ngoài phạm vi. Frontend production build đạt; chưa nghiệm thu trực tiếp trên browser.
- 2026-09-25: Khắc phục bổ sung Checkout các dòng 235–237/239/240: Payment date cho phép gõ trực tiếp, có icon lịch riêng và giữ giới hạn ngày đến–ngày đi/ngày hệ thống; giờ/ca chỉ dùng cấu hình hợp lệ, tự ghép ca khi sửa giờ, phân biệt đổi ca thủ công và backend từ chối khi chưa cấu hình ca; bổ sung giải thích hai chế độ chỉnh/bổ sung tiền phòng; No Post kiểm tra cả Booking và phòng ở các API post FO/HK/tiền phòng/điều chỉnh giá/no-show. Không đổi Night Audit, dữ liệu, migration hoặc dòng 238. Frontend build và PHP lint đạt; backend feature tests chưa chạy được vì runtime báo connection `mysql_data` chưa cấu hình; chưa nghiệm thu trực tiếp trên browser.
